...
class ItemsController {
  ...
  private function validate(string $name, string $qtyRaw, ?string $categoryIdRaw, ?int &$qtyOut = null, ?int &$categoryIdOut = null): array {
    ...
    if ($qtyRaw === '' || !ctype_digit($qtyRaw)) {
      ...
      $q = (int)$qtyRaw;
      if ($q < 0 || $q > 9999) {
        $errors['qty'] = 'Cantidad fuera de rango (0..9999).';
      }
    }
  ...
  public function apiIndex(): void {
    ...
    $q = Item::with('category')->orderBy('id', 'desc');
    if ($all !== '1') {
      $q->where('is_active', 1);
    }
    $items = $q->get();
  ...
  public function apiCreate(): void {
    ...
    $item = Item::create(['name' => trim($name), 'qty' => $qty, 'category_id' => $categoryId, 'is_active' => 1]);
    $item->load('category');
  ...
  public function apiUpdate(int $id): void {
    ...
    if (!$item) {
      response_json(['ok' => false, 'err' => 'Item no encontrado'], 404);
      return;
    }
    ...
    response_json(['ok' => true, 'item' => $this->itemToApi($item)], 200);
  }
  ...
  public function apiDelete(int $id): void {
    ...
    $item->is_active = 0;
    $item->save();
    response_json(['ok' => true, 'id' => $id, 'deleted' => true], 200);
  }
