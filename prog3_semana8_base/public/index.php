...
try {
  ...
  // Semana 8: Update / Delete (soft delete)
  if ($method === 'PUT' && preg_match('~^/api/items/(\\d+)$~', $uri, $m)) {
    (new ItemsController())->apiUpdate((int)$m[1]);
    exit;
  }

  if ($method === 'DELETE' && preg_match('~^/api/items/(\\d+)$~', $uri, $m)) {
    (new ItemsController())->apiDelete((int)$m[1]);
    exit;
  }

  // Semana 7: categorías (relación 1:N)
  if ($method === 'GET' && $uri === '/api/categories') {
    (new CategoriesController())->apiIndex();
    exit;
  }

  if ($method === 'GET' && preg_match('~^/api/categories/(\\d+)/items$~', $uri, $m)) {
    (new CategoriesController())->apiItemsByCategory((int)$m[1]);
    exit;
  }
